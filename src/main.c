...
static void draw_digit(GContext *ctx, int digit, int x, int y){
  ...
  for (int i=0; i<5; i++){
    ...
    for (int j=0; j<3; j++){
       ...
       // second to access the individual character - either a 0 or 1 - from the string in that element
...
static void canvas_update_proc(Layer *this_layer, GContext *ctx) {
  ...
  int digit_height = (5 * tile_size) + 4;
    ...
    // top left
    draw_digit(ctx, 1, s_main_window_center.x-digit_width-(tile_size/2), s_main_window_center.y-digit_height-(tile_size/2));  
  
    // top right
    draw_digit(ctx, 2, s_main_window_center.x+(tile_size/2), s_main_window_center.y-digit_height-(tile_size/2));  
    
    // bottom left
    draw_digit(ctx, 3, s_main_window_center.x-digit_width-(tile_size/2), s_main_window_center.y+(tile_size/2));  
  
    // bottom right
    draw_digit(ctx, 4, s_main_window_center.x+(tile_size/2), s_main_window_center.y+(tile_size/2));  
